...
def read_data(data_dir, fold, rd=False):
    if rd:
        data_tr = pd.read_csv(os.path.join(data_dir, f'index_data_label_lost_{fold}_train.csv'), header=None)
        data_te = pd.read_csv(os.path.join(data_dir, f'index_data_label_lost_{fold}_test.csv'), header=None)
        data_tr = fillna_rd(data_tr)
        data_te = fillna_rd(data_te)
    else:
        data_tr = pd.read_csv(os.path.join(data_dir, f'train{fold}.csv'), header=None)
        data_te = pd.read_csv(os.path.join(data_dir, f'test{fold}.csv'), header=None)
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            if np.isnan(data.iloc[i, j]):
                data.iloc[i, j] = np.random.random(1)
    return data
