"""
Batch run AutoMetric for nat1-DA, nat1-DS, nat2-DS
"""
import numpy as np
import pandas as pd
import argparse
...
def main():
    ...
    AutoMetric_drop(args)


def AutoMetric_drop(args):
    data_name, drop = args.data_name.split('_drop')
    drop = 'DA' if drop == 'Feature' else 'DS'
    rand_list = rand_list_dict[data_name]
    para_name0 = 'N3s10b64dec300lr0.01it500'
    dims = {
        'nat1_dropFeature': '--feature_num 638 --w_feature_num 634 ',
        'nat1_dropSample': '--feature_num 669 --w_feature_num 665 ',
        'nat2_dropFeature': '--feature_num 630 --w_feature_num 626 ',
    }
    cmdRoot = ' '.join([
        'python ./../comparing/AutoMetric/main.py',
    ...
    cmd_list = []
    have, total = 0, 0
    seed_list = get_seed(f'{drop}+AutoMetric')[data_name]
    for (rand, seed), fold in product(zip(rand_list, seed_list), range(5)):
        exp_name = f'{para_name0}se{seed}__'
        total += 1
        if not fail_flag(os.path.join(args.save_dir, 'AutoMetric'),
